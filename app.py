...
def get_greeting_response(question: str = "", lang: str = "en") -> str:
    ...
    if lang == "fr":
        return (
            f"{opener}! Je suis votre Assistant Pharmacie Sunrise. "
...
def _out_of_scope_response(lang: str = "en") -> str:
    global _oos_idx
...
def route_and_respond(question: str, conversation_history=None, lang: str = "en"):
    """
    ...
    if tool == "out_of_scope":
        return _out_of_scope_response(lang), "", "system"
...
def respond(message, chat_history, search_history, lang="en"):
    """
    ...
    try:
        answer, source, mode = route_and_respond(message, conversation_history, lang)
...
def drug_summary_respond(drug_name, chat_history, search_history, lang="en"):
    if not drug_name:
        return chat_history, search_history, gr.update(), gr.update(), ""
    try:
...
def click_quick_question(question, chat_history, search_history, lang="en"):
    return respond(question, chat_history, search_history, lang)


def reask_from_history(selected_question, chat_history, search_history, lang="en"):
    if not selected_question:
        return "", chat_history, search_history, gr.update(), gr.update(), ""
    return respond(selected_question, chat_history, search_history, lang)
...
with gr.Blocks(title="Netrisyl Pharmacy Assistant") as demo:
    ...
    with gr.Row():
        ...
        # ── LEFT sidebar — Language + Drug Lookup ─────────────────
        with gr.Column(scale=1, min_width=200):
            ...
            drug_search   = gr.Textbox(placeholder="Type e.g. amox... / Tapez ex. amox...", label="Search / Rechercher")
            drug_dropdown = gr.Dropdown(choices=DRUG_NAMES[:20], label="Select drug / Sélectionner", interactive=True)
            drug_lookup_btn = gr.Button("📋 Get Summary / Voir Fiche", variant="primary", size="sm")
            gr.Markdown("---")
            gr.Markdown("""
...
- 🧪 *Clinical* — AI summary + disclaimer
        ...
        with gr.Column(scale=3, min_width=400):
            ...
            with gr.Row():
                msg    = gr.Textbox(
                    placeholder="Ask e.g. 'Do we have Amoxicillin?' / Demandez ex. 'Avons-nous de l\'Amoxicillin?'",
                    label="",
                    scale=4
                )
    ...
    submit.click(respond,
        [msg, chatbot, search_history_state, lang_state],
    ...
    # ── Voice input ──────────────────────────────────────────────────
    def transcribe_audio(audio_path, chat_history, search_history, lang="en"):
        if not audio_path:
            return "", chat_history, search_history, gr.update(), gr.update(), gr.update(value=None), ""
        try:
            with open(audio_path, "rb") as f:
                transcript = client.audio.transcriptions.create(model="whisper-1", file=f)
            result = respond(transcript.text, chat_history, search_history, lang)
    ...
    history_dropdown.change(reask_from_history,
        [history_dropdown, chatbot, search_history_state, lang_state],
        [msg, chatbot, search_history_state, history_dropdown, history_display, brief_box])

    # ── Quick question buttons ───────────────────────────────────────
    for btn, question in zip(quick_btns, QUICK_QUESTIONS):
        btn.click(click_quick_question,
            [gr.Textbox(value=question, visible=False), chatbot, search_history_state, lang_state],
            [msg, chatbot, search_history_state, history_dropdown, history_display, brief_box])
    ...
    def do_brief(chat_history, lang="en"):
        if not chat_history:
            return "No response yet — ask a question first." if lang == "en" else "Pas encore de réponse."
        try:
            last_response = ""
            ...
            response = client.chat.completions.create(
                ...
                messages=[{
                    "role": "user",
                    "content": (
                        f"Summarise the following pharmacy data in 2-3 clear sentences "
                        f"for a pharmacy manager. Numbers and actionable insights only. "
                        f"No bullet points or markdown. {lang_instr}\n\nResponse:\n"
                        + last_response[:1500]
                        + "\n\nBrief summary:"
                    )
                }],
